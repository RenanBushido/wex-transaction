...
public class PurchaseTransactionTests
{
    ...
    {
        var transaction = PurchaseTransaction.Create(_validDescription, ValidDate, ValidAmount);
    ...
    {
        var t1 = PurchaseTransaction.Create(_validDescription, ValidDate, ValidAmount);
        var t2 = PurchaseTransaction.Create(_validDescription, ValidDate, ValidAmount);
    ...
    public void Create_WithZeroAmount_ThrowsInvalidAmountException()
    {
        Assert.Throws<InvalidAmountException>(() =>
            PurchaseTransaction.Create(_validDescription, ValidDate, 0m));
    }
    ...
    {
        Assert.Throws<InvalidAmountException>(() =>
            PurchaseTransaction.Create(_validDescription, ValidDate, -5.00m));
    }
}
