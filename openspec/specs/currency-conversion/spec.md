...
Defines the domain rules for currency conversion of purchase transactions. The `ExchangeRate` value object encapsulates a validated rate record from the Treasury Reporting Rates of Exchange dataset. The `ExchangeRateSelector` domain service selects the most recent applicable rate within a 6-month window and computes the converted amount. When no qualifying rate is available, a domain exception is raised and surfaced as HTTP 422.
