...
Defines the behavior of the `GlobalExceptionHandler` in the `WexTransaction.Api` project. The handler maps domain exceptions to RFC 7807 `ProblemDetails` HTTP responses, ensuring consistent error shapes for clients while leaving non-domain exceptions to ASP.NET Core's default 500 fallback.
